...
uses
  System.Types, System.Classes, Vcl.ImgList, System.StrUtils, Winapi.Windows,
...
{$R cxGridMenuOperations_svg.res}

uses
  System.SysUtils, Vcl.Graphics, cxGridPopupMenuConsts, cxGraphics, cxLookAndFeels, dxGDIPlusClasses, dxSVGImage;

const
  dxThisUnitName = 'cxGridMenuOperations';

const
  SVGGroupMenuIcons: array [TcxSummaryKind] of string = (
    '', 'MenuFooterSum', 'MenuFooterMin', 'MenuFooterMax', 'MenuFooterCount', 'MenuFooterAverage', '');
...
function GetSummaryImageResourceName(ASummaryKind: TcxSummaryKind): string;
begin
  if ASummaryKind in [skNone, skOrder] then
    Result := ''
  else
  begin
    if cxLookAndFeels.dxUseVectorIcons then
      Result := SVGGroupMenuIcons[ASummaryKind]
    else
...
var
  AImageResourceName: string;
  AImage: TdxSmartGlyph;
begin
  Result := -1;
  AImageResourceName := GetImageResourceName;
  if AImageResourceName = '' then Exit;
  if cxGridPopupMenuImages = nil then
    cxGridPopupMenuImages := TcxImageList.CreateSize(16, 16);
  Result := OperationImageResourceNames.IndexOf(AImageResourceName);
  if Result = -1 then
  begin
    AImage := TdxSmartGlyph.Create;
    try
      if dxUseVectorIcons then
        AImage.LoadFromResource(GetInstance, UpperCase(AImageResourceName), 'SVG')
      else
        AImage.LoadFromResource(GetInstance, UpperCase(AImageResourceName), RT_RCDATA);
      TcxImageList(cxGridPopupMenuImages).Add(AImage, AImageResourceName);
    finally
      AImage.Free;
    end;
    ...
    OperationImageResourceNames[Result] := AImageResourceName;
  end;
end;
