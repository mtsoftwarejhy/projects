...
{           ExpressQuantumGrid                                       }
{                                                                    }
{           Copyright (c) 2001-2026 Developer Express Inc.           }
{           ALL RIGHTS RESERVED                                      }
{                                                                    }
...
uses
  System.UITypes,
{$IFNDEF NONDB}
  Data.DB, Data.FMTBcd, Data.SqlTimSt, cxDBData, cxGridDBDataDefinitions,
{$ENDIF}
  System.Types, System.Variants, Winapi.Windows, System.Classes, System.SysUtils, Vcl.Graphics, System.Contnrs, System.Generics.Defaults, System.Generics.Collections,
...
type
  ...
  strict private
    ...
    class function RevertScaling(AValue: Integer; AScaleFactor: TdxScaleFactor): Integer; overload; static; inline;
    //expand
  ...
  protected
    ...
    function RevertScaling(AValue: Integer): Integer; overload; inline;
    function RevertScaling(const AValue: TRect): TRect; overload; inline;
  ...
  public
    constructor Create(AOwner: TcxGridCustomTableViewExport; AViewInfo: TdxLayoutContainerViewInfo); virtual; // for internal use
  ...
  public
    ...
    procedure Commit(AExportProvider: IcxExportProvider; AExport: TcxGridCustomExport);
...
uses
  System.Math, System.RTLConsts, Vcl.Dialogs, cxGridCustomLayoutView, cxImageComboBox, dxTypeHelpers,
...
function TcxGridCustomExport.RevertScaling(AValue: Integer): Integer;
var
  AScaleFactor: TdxScaleFactor;
begin
  AScaleFactor := TcxCustomGridAccess(Grid).ScaleFactor;
  if AScaleFactor.Assigned then
    Result := RevertScaling(AValue, AScaleFactor)
  else
    Result := AValue;
end;

function TcxGridCustomExport.RevertScaling(const AValue: TRect): TRect;
var
  AScaleFactor: TdxScaleFactor;
begin
  AScaleFactor := TcxCustomGridAccess(Grid).ScaleFactor;
  if AScaleFactor.Assigned then
    Result := Rect(
      RevertScaling(AValue.Left, AScaleFactor), RevertScaling(AValue.Top, AScaleFactor),
      RevertScaling(AValue.Right, AScaleFactor), RevertScaling(AValue.Bottom, AScaleFactor))
  else
    Result := AValue;
end;

class function TcxGridCustomExport.RevertScaling(AValue: Integer; AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := AScaleFactor.Revert(AValue);
  if (Result = 0) and (AValue <> 0) then
    Result := Sign(AValue);
end;
...
begin
  ...
  begin
    ...
    MainCalculate(ScaleFactor.Apply(ABounds));
  end;
end;
...
begin
  if Safe.Cast<TcxCustomGridTableView>(GridView, AGridView) then
  begin
    Grid.BeginUpdate;
    try
...
begin
  ...
  if AViewParams.Font.PixelsPerInch <> 0 then
    AExportStyle.FontSize := RevertScaling(AViewParams.Font.Size * AViewParams.Font.PixelsPerInch div dxDefaultDPI)
  else
...
begin
  ...
  if Safe.Cast<TcxGridMasterDataRow>(ARecord, AMasterDataRow) then
  begin
    if Safe.Cast<TcxCustomGridTableView>(AMasterDataRow.ActiveDetailGridView, AActiveDetailView) then
...
begin
  Result := ViewInfo.LookAndFeelPainter.ScaledExpandButtonSize(dxDefaultScaleFactor);
end;
...
var
  ...
  AStyle: TcxCacheCellStyle;
begin
  ABounds := GetRelativeBounds(FOwner.RevertScaling(AViewInfo.EditViewInfo.Bounds));
...
begin
  ARect := GetRelativeBounds(FOwner.RevertScaling(AViewInfo.Bounds));
  ...
  begin
    Inc(ARect.Top, FOwner.RevertScaling(AViewInfo.CaptionViewInfo.TextAreaBounds).Height div 2);
    AExportItem := Owner.AddVisualItemEx(ARect, '',  AParams, taLeftJustify, [], clDefault, True);
  end;
...
begin
  Owner.AddVisualItemEx(GetRelativeBounds(FOwner.RevertScaling(AViewInfo.CaptionViewInfo.Bounds)), AViewInfo.CaptionViewInfo.Text,
...
begin
  Result := cxRectOffset(cxRectOffset(ABounds, FOwner.RevertScaling(ViewInfo.ContentBounds).TopLeft, False), Position);
end;
...
begin
  ...
  try
    ...
    try
      ...
      RecordHeight := RevertScaling(AViewInfo.DataHeight);
      if AViewInfo.HasPreview then
        Dec(FRecordHeight, RevertScaling(AViewInfo.PreviewViewInfo.Height));
...
var
  ...
  AExportItem: TcxExportVisualItem;
begin
  AHeight := RevertScaling(ViewInfo.RecordsViewInfo.GroupRowHeight);
...
begin
  ...
  R := Rect(ALevel * RevertScaling(ViewInfo.LevelIndent), ATop, (ALevel + 1) * RevertScaling(ViewInfo.LevelIndent), ATop + ARowHeight);
  ...
  begin
    ...
    OffsetRect(R, -RevertScaling(ViewInfo.LevelIndent), 0);
  end;
  Result := (ALevel + 1) * RevertScaling(ViewInfo.LevelIndent);
end;
...
begin
  ...
  begin
    ABounds := Rect((ARow.Level + 1) * RevertScaling(ViewInfo.LevelIndent), ATop, RecordWidth, ATop + DefaultRowHeight);
    AVisualItem := AddVisualDataItem(ABounds, DefaultStyleIndex, ARow, nil);
    AVisualItem.DetailViewViewInfo := CalculateViewViewInfo(ARow.ActiveDetailGridView, RevertScaling(ViewInfo.Bounds));
...
begin
  ...
  AExportItem := AddVisualItemEx(R, '', AParams, AColumn.GetProperties.Alignment.Horz, GridLines, GridLineColor);
  ATop := R.Bottom;
  AExportItem.Data := ARow;
  AExportItem.Data2 := AColumn;
end;
...
begin
  FRecordWidth := RevertScaling(ViewInfo.DataWidth);
...
begin
  if FSafeLeftPos <> 0 then
    LeftPos := FSafeLeftPos;
...
var
  ...
  AVisualItem: TcxExportVisualItem;
begin
  AOffsetY := ATop - RevertScaling(AViewInfo.Bounds).Top;
  APrevTop := cxMaxRectSize;
  for I := 0 to AViewInfo.Count - 1 do
  begin
    AItem := AViewInfo.Items[I];
    AItem.Recalculate;
    R := cxRectOffset(RevertScaling(AItem.RealBounds), GetColumnOffset(AItem.Column), AOffsetY);
...
begin
  ...
  if RecordHeight = 0 then
    RecordHeight := RevertScaling(ViewInfo.RecordsViewInfo.RowHeight);
...
begin
  if ViewInfo.IndicatorViewInfo.Visible then
    Result := RevertScaling(ViewInfo.IndicatorViewInfo.Width)
  else
    Result := 0;
end;
...
begin
  Result := RevertScaling(GridView.Controller.LeftPos);
end;
...
begin
  GridView.Controller.LeftPos := ViewInfo.ScaleFactor.Apply(AValue);
...
begin
  AHeight := 0;
  AOffsetX := 0;
  AOffsetY := ATop - RevertScaling(ViewInfo.HeaderViewInfo.Bounds).Top;
  ...
  begin
    ...
    if not AForRootBands then
      AOffsetX := RevertScaling(ABandViewInfo.Bounds).Left + GetParentBandOffset(ABandViewInfo.Band.ParentBand);
    with cxRectSize(RevertScaling(ABandViewInfo.Bounds)) do
      R := cxRectBounds(AOffsetX, RevertScaling(ABandViewInfo.Bounds).Top + AOffsetY, cx, cy);
    ...
    AColsOffset := AOffsetX - RevertScaling(ABandViewInfo.Bounds).Left;
    AddPattern(R, ABandViewInfo.Band, AColsOffset);
    ABandHeader := ABandViewInfo.HeaderViewInfo;
    R := cxRectOffset(cxRectSetLeft(RevertScaling(ABandHeader.Bounds), AOffsetX), 0, AOffsetY);
    if (ABandHeader.Band.FixedKind = fkRight) and IsFirstInGroup(ABandHeader.Band) then
      OffsetRect(R, RevertScaling(OptionsView.FixedBandSeparatorWidth), 0);
    ...
    if AForRootBands then
      Inc(AOffsetX, cxRectWidth(RevertScaling(ABandViewInfo.Bounds)));
  end;
  ATop := AHeight;
end;
...
begin
  ...
  if ViewInfo.GroupByBoxViewInfo.Visible then
    Result.Y := -RevertScaling(ViewInfo.GroupByBoxViewInfo.Bounds).Bottom;
end;
...
begin
  ...
  Dec(ATop, RevertScaling(AContainer.Bounds).Top);
  Dec(ALeft, RevertScaling(AContainer.Bounds).Left);
  R := RevertScaling(AContainer.Bounds);
  AddVisualItemEx(cxRectOffset(RevertScaling(AContainer.Bounds), ALeft, ATop),
...
begin
  Result := FOwner.Owner.RevertScaling(Owner.Owner.OptionsView.CategorySeparatorWidth);
end;
...
begin
  ...
  if ARow.HasIndent then
    Result := FOwner.RevertScaling(Owner.OptionsView.CategoryIndent)
  else
    Result := 0;
...
begin
  ...
  if ACellViewInfo <> nil then
    Result.Bounds := cxRectSetHeight(Result.Bounds, FOwner.RevertScaling(ACellViewInfo.ContentHeight));
...
begin
  Result := FOwner.RevertScaling(Owner.OptionsView.CardBorderWidth);
end;
...
begin
  Result := FOwner.RevertScaling(Owner.OptionsView.LayerSeparatorWidth);
end;
...
begin
  if LayoutDirection = ldHorizontal then
    Adapter.AddRangeHorz(APosition, APosition + RevertScaling(OptionsView.SeparatorWidth))
  else
    Adapter.AddRangeVert(APosition, APosition + RevertScaling(OptionsView.SeparatorWidth));

  FCardSeparators.AddPairs(APosition, APosition + RevertScaling(OptionsView.SeparatorWidth));
end;
  ...
  try
    ...
    begin
      ...
      for I := 0 to AWidths.Count - 1 do
        AWidths[I] := RevertScaling(GridView.OptionsView.CaptionWidth);
    end;
    for I := 0 to CardCount - 1 do
...
begin
  ...
  try
    ABounds := RevertScaling(Bounds);
    if LayoutDirection = ldVertical then
      ColumnCardCount := Min(CardCount, Max(1, (ABounds.Right - ABounds.Left) div CardWidth))
    else
      RowCardCount := Min(CardCount, Max(1, (ABounds.Bottom - ABounds.Top) div CardHeight));
  except
    on EDivByZero do;
  end;
  if LayoutDirection = ldHorizontal then
...
begin
  Result := RevertScaling(OptionsView.CardBorderWidth);
end;
...
begin
  Result := RevertScaling(GridView.ViewInfo.RecordsViewInfo.RowHeight);
end;

function TcxGridCardViewExport.GetCardIndent: Integer;
begin
  Result := RevertScaling(OptionsView.CardIndent);
end;
...
var
  R: TRect;
begin
  Result := RevertScaling(OptionsView.CardWidth);
  if OptionsView.CardAutoWidth then
  begin
    R := RevertScaling(GridView.ViewInfo.Bounds);
...
begin
  Result := RevertScaling(OptionsView.CategoryIndent);
end;

function TcxGridCardViewExport.GetCategorySeparatorWidth: Integer;
begin
  Result := RevertScaling(GridView.OptionsView.CategorySeparatorWidth);
end;
...
begin
  Result := CardIndent * 2;
  if LayoutDirection = ldHorizontal then
    Inc(Result, RevertScaling(OptionsView.SeparatorWidth));
end;
...
begin
  Result := RevertScaling(GridView.OptionsView.LayerSeparatorWidth);
end;
...
begin
  inherited DoCreateExportCells;
  ARecordWidth := RevertScaling(GridView.ViewInfo.RecordsViewInfo.RecordWidth);
  ACardsBounds := RevertScaling(GridView.ViewInfo.RecordsViewInfo.Bounds).Width;
  ...
  try
    ...
    begin
      ...
      try
        ACardViewInfo.Calculate(-10000, ViewInfo.ScaleFactor.Apply(ACardBounds.Top));
        ACardBounds := cxRectSetSize(ACardBounds, RevertScaling(ACardViewInfo.Width), RevertScaling(ACardViewInfo.Height));
        ...
        begin
          ...
          Inc(ATop, RevertScaling(ACardViewInfo.CaptionViewInfo.Height));
        end;
        ExportCardLayout(ACardViewInfo, ATop, ACardBounds.Left);
        ACardsRowHeight := Max(ACardsRowHeight, RevertScaling(ACardViewInfo.Height));
        ACardBounds.Left := ACardBounds.Right + RevertScaling(GridView.OptionsView.RecordIndent);
        if (ACard + 1) mod AColumnCardCount = 0 then
        begin
          OffsetRect(ACardBounds, 0, ACardsRowHeight + RevertScaling(GridView.OptionsView.RecordIndent));
          ACardBounds.Left := 0;
          ACardsRowHeight := 0;
        end;
      finally
...
begin
  ...
  ARect := cxRectOffset(cxRectOffset(RevertScaling(AInfo.Bounds), RevertScaling(AInfo.RecordViewInfo.Bounds).TopLeft, False), ALeft, ATop);
...
begin
  ...
  AddVisualDataItem(RevertScaling(AViewInfo.Bounds), AStyleIndex, AViewInfo.GridRecord, AViewInfo.Item);
end;
...
begin
  ...
  AddVisualItemEx(RevertScaling(AViewInfo.Bounds), AViewInfo.Text, AViewParams, taLeftJustify, cxBordersAll);
end;
...
begin
  inherited DoCreateExportCells;
  APrevHeight := 0;
  AContentWidth := RevertScaling(GridView.ViewInfo.ClientWidth);
  ...
  try
    ...
    begin
      ...
      try
        if (I > 0) and ((APos.X + RevertScaling(ARecordViewInfo.Width) > AStartPos.X + AContentWidth) or
          (ARecord.IsData <> Records[I - 1].IsData)) then
        begin
          APos.X := AStartPos.X;
          Inc(APos.Y, APrevHeight + IndentBetweenItems);
        end;
        ARecordViewInfo.MainCalculate(ViewInfo.ScaleFactor.Apply(APos.X), ViewInfo.ScaleFactor.Apply(APos.Y));
        AddExportRecord(ARecord, ARecordViewInfo);
        Inc(APos.X, RevertScaling(ARecordViewInfo.Width) + IndentBetweenItems);
        APrevHeight := RevertScaling(ARecordViewInfo.Height);
      finally
...
begin
  ...
  Adapter.AddBounds(cxRect(0, 0, RevertScaling(AWidth), RevertScaling(AHeight)));
end;
...
begin
  ...
  if ProviderCache <> nil then
    ConditionalFormattingRules.Commit(ProviderCache.Provider, Self);
end;
...
const
  ASubTotalKinds: array[TcxSummaryKind] of Char = ('0', '9', '5', '4', '3', '1', '0');
var
  I, J: Integer;
  AArea: TRect;
begin
  Result := '';
  if AKind in [skNone, skOrder]  then
    Exit;
...
procedure TcxGridDataConditionalFormattingRules.Commit(AExportProvider: IcxExportProvider; AExport: TcxGridCustomExport);
var
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          AFormatter.Anchors := AExport.RevertScaling(ARuleAreas.BoundingRect).TopLeft;
...
begin
  ...
  if (FooterSubTotalFormulaBuilder.Count > 0) and not (ASummaryItem.Kind in [skNone, skCount, skOrder]) then
  begin
