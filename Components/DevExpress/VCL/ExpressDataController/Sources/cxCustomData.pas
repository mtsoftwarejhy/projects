...
//    function GetRecordCount: Integer; virtual;
  ...
  protected
    ...
    function GetTextStartPosition(const AText: string; out AHighlightedText: string): Integer; overload; // deprecated 'Use overload version instead';
    function GetTextStartPosition(const AText: string; out AHighlightedLength: Integer): Integer; overload;
  ...
  public
    ...
    function GetTextStartPositionByRecordIndex(ARecordIndex: Integer; AItemIndex: Integer;
      out AHighlightedText: string): Integer; overload; deprecated 'Use overload version instead';
    function GetTextStartPositionByRowIndex(ARowIndex: Integer; AItemIndex: Integer;
      out AHighlightedText: string): Integer; overload; deprecated 'Use overload version instead';

    function GetTextStartPositionByRecordIndex(ARecordIndex: Integer; AItemIndex: Integer;
      out AHighlightedLength: Integer): Integer; overload;
    function GetTextStartPositionByRowIndex(ARowIndex: Integer; AItemIndex: Integer;
      out AHighlightedLength: Integer): Integer; overload;
  ...
  TcxSummaryKind = (skNone, skSum, skMin, skMax, skCount, skAverage, skOrder);
  ...
  private
    ...
    function IsLinksStored: Boolean;
    function IsSummaryItemsStored: Boolean;
  public
  ...
  published
    property Links: TcxDataSummaryGroupItemLinks read GetItemLinks write SetItemLinks stored IsLinksStored;
    property SummaryItems: TcxDataGroupSummaryItems read FSummaryItems write SetSummaryItems stored IsSummaryItemsStored;
  end;
  ...
  private
    ...
    function IsDefaultGroupSummaryItemsStored: Boolean;
    function IsFooterSummaryItemsStored: Boolean;
    function IsSummaryGroupsStored: Boolean;
  protected
  ...
  published
    property DefaultGroupSummaryItems: TcxDataGroupSummaryItems read FDefaultGroupSummaryItems write SetDefaultGroupSummaryItems stored IsDefaultGroupSummaryItemsStored;
    property FooterSummaryItems: TcxDataFooterSummaryItems read FFooterSummaryItems write SetFooterSummaryItems stored IsFooterSummaryItemsStored;
    property SummaryGroups: TcxDataSummaryGroups read FSummaryGroups write SetSummaryGroups stored IsSummaryGroupsStored;
...
//FIsResetHasChildrenFlag: Boolean;
  ...
  protected
    ...
    procedure RestructureData; virtual;
  ...
  public
    ...
    property InDeleteDetailRecord: Boolean read FInDeleteDetailRecord write FInDeleteDetailRecord; // for internal use
...
const
  ...
  ADateFormats: array [Boolean, skSum..skAverage] of string = (
    ('%s=', '%s=', '%s=', '%s=0', '%s='),
    ('', '%s=', '%s=', '0', '%s='));
  AFloatFormats: array [Boolean, skSum..skOrder] of string = (
...
begin
  ...
  begin
    if (Kind <> skNone) and (Kind <> skOrder) then
    begin
...
function TcxDataSummaryGroup.IsLinksStored: Boolean;
begin
  Result := Links.Count <> 0;
end;

function TcxDataSummaryGroup.IsSummaryItemsStored: Boolean;
begin
  Result := SummaryItems.Count <> 0;
end;
...
begin
  ...
  case ASummaryItem.Kind of
    ...
    skOrder:
      SummaryValue := IntToStr(DataController.FocusedRowIndex + 1) + '/' +
        IntToStr(CountValue);
  end;
end;
...
function TcxDataSummary.IsDefaultGroupSummaryItemsStored: Boolean;
begin
  Result := DefaultGroupSummaryItems.Count <> 0;
end;

function TcxDataSummary.IsFooterSummaryItemsStored: Boolean;
begin
  Result := FooterSummaryItems.Count <> 0;
end;

function TcxDataSummary.IsSummaryGroupsStored: Boolean;
begin
  Result := SummaryGroups.Count <> 0;
end;
...
function TcxDataFindCriteria.GetTextStartPositionByRecordIndex(ARecordIndex: Integer;
  AItemIndex: Integer; out AHighlightedLength: Integer): Integer;
var
  ADisplayText: string;
  AField: TcxCustomDataField;
begin
  Result := -1;
  AHighlightedLength := 0;
  AField := DataController.Fields[AItemIndex];
  ...
  Result := GetTextStartPosition(ADisplayText, AHighlightedLength);
end;
...
begin
  ...
  begin
    ...
    Result := TdxDiacriticStringOptions.AnsiPos(ASearchText, ADisplayText);
    Dec(Result);
    if Result <> -1 then
    begin
      AHighlightedText := ASearchText;
      Break;
    end;
  end;
end;

function TcxDataFindCriteria.GetTextStartPosition(const AText: string; out AHighlightedLength: Integer): Integer;
var
  I: Integer;
  ASearchText, ADisplayText: string;
  ACondition: TcxDataFindCriteriaCondition;
begin
  ...
  begin
    ...
    if IsConditionsLowerCase then
      ...
      ASearchText := AnsiLowerCase(ACondition.Text);
    TdxDiacriticStringOptions.AnsiPosAndLength(ASearchText, ADisplayText, Result, AHighlightedLength);
    Dec(Result);
    if Result <> -1 then
      Break;
  end;
...
procedure TcxCustomDataController.RestructureData;
begin
  BeginFullUpdate;
  try
    Provider.ResetEditing;
...
var
  ARootDataController: TcxCustomDataController;
begin
  if (DetailMode = dcdmClone) and IsDetailMode and not IsSmartRefresh then
  begin
    ARootDataController := GetRootDataController;
    if (ARootDataController.LockCount > 0) and not Assigned(ARootDataController.FSaveObject) then
    begin
      ARootDataController.FPostSyncMasterPosDataController := Self;
      Exit;
    end;
  end;
