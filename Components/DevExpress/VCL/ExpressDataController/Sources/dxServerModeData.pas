...
uses
  System.Types, Winapi.Windows, System.SysUtils, System.Classes, System.Variants, Data.DB, dxCore, cxClasses, dxCoreClasses,
...
type
  ...
  TdxServerModeDataFindCriteria = class;

  TdxNullsSortOrder = (nsoFirstIfDesc, nsoFirstIfAsc);

  IdxProviderSupport = IProviderSupportNG;

  { TdxServerModeFindCriteria }

  TdxServerModeFindCriteria = class(TcxCustomDataFindCriteria)
  private
  ...
  public
    procedure AssignConditions(AFindCriteria: TdxServerModeFindCriteria);
    function IsEqual(AFindCriteria: TdxServerModeFindCriteria): Boolean;
  end;
  ...
  private
    ...
    FFindCriteria: TdxServerModeFindCriteria;
  ...
  protected
    ...
    function CreateFindCriteria: TdxServerModeFindCriteria;
    ...
    procedure Apply(AFilter: TdxServerModeCriteria; AFindCriteria: TdxServerModeFindCriteria;
    ...
    property FindCriteria: TdxServerModeFindCriteria read FFindCriteria;
  ...
  private
    FKey: Variant;
    FIsGroupKey: Boolean;
    FDataExpandingInfo: TdxFastObjectList;
  ...
  public
    ...
    property InternalTopRecordIndex: integer read FInternalTopRecordIndex write FInternalTopRecordIndex;
  end;
  ...
  private
    ...
    procedure ConvertFindCriteria(ADestination: TdxServerModeFindCriteria; ASource: TdxServerModeDataFindCriteria);
  ...
  protected
    ...
    procedure ApplyRecoveryGroupInfo(AGroup: TdxServerModeDataGroupInfo);
  ...
  TdxServerModeDataFindCriteria = class(TcxDataFindCriteria)
  private
    function GetDataController: TdxServerModeDataController;
  protected
  ...
  TdxServerModeRecoveryStateEvent = procedure (ADataFocusingInfo: TdxServerModeDataFocusingInfo) of object;
  ...
  private
    ...
    FOnGetRecoveryState: TdxServerModeRecoveryStateEvent;
    FOnSetRecoveryState: TdxServerModeRecoveryStateEvent;
    function GetFindCriteria: TdxServerModeDataFindCriteria;
  ...
  protected
    ...
    function GetInternalErrorCode(ARecordIndex: Integer; AItemIndex: Integer): Integer; override;
    ...
    procedure PopulateFilterValues(AList: TcxDataFilterValueList;
      AItemIndex: Integer; ACriteria: TcxFilterCriteria; var AUseFilteredRecords: Boolean; out ANullExists: Boolean;
      AUniqueOnly: Boolean; AFilteredValuesShowFilteredItemsOnly: Boolean); override;

    property FieldCount: Integer read GetFieldCount;
    property Fields[Index: Integer]: TdxServerModeDataField read GetField;

    property OnGetRecoveryState: TdxServerModeRecoveryStateEvent read FOnGetRecoveryState write FOnGetRecoveryState;
    property OnSetRecoveryState: TdxServerModeRecoveryStateEvent read FOnSetRecoveryState write FOnSetRecoveryState;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    ...
    property FindCriteria: TdxServerModeDataFindCriteria read GetFindCriteria;
...
uses
  System.RTLConsts, System.Math, cxDateUtils, System.StrUtils, dxServerModeConsts, dxServerModeSQLAdapters,
  System.DateUtils;
...
type
  ...
  { TdxServerModeDataExpandingInfo }

  TdxServerModeDataExpandingInfo = class(TcxDataExpandingInfo)
  public
    ParentGroupValue: Variant;
    Level: Integer;
  end;
...
procedure TdxServerModeFindCriteria.AssignConditions(AFindCriteria: TdxServerModeFindCriteria);
begin
  Filter.Root.Assign(AFindCriteria.Filter.Root);
end;

function TdxServerModeFindCriteria.IsEqual(AFindCriteria: TdxServerModeFindCriteria): Boolean;
begin
  Result := ((AFindCriteria = nil) and Filter.IsEmpty) or
    ((AFindCriteria <> nil) and Filter.IsEqual(AFindCriteria.Filter));
end;

function TdxServerModeFindCriteria.CreateFilter: TcxFilterCriteria;
begin
  Result := DataSource.CreateCriteria;
end;

procedure TdxServerModeFindCriteria.SetEscapedFlagForFilterItems;
...
function TdxServerModeFindCriteria.GetDataSource: TdxServerModeCustomDataSource;
begin
  Result := TdxServerModeCustomDataSource(inherited Owner);
end;

function TdxServerModeFindCriteria.GetFilter: TdxServerModeCriteria;
begin
...
function TdxServerModeKeyedCache.GetRowIndexByKey(const AKey: Variant): Integer;
  ...
  function AreAllKeysFetched(AGroupInfo: TdxServerModeGroupInfo): Boolean;
  var
    I: Integer;
  begin
    Result := True;
...
begin
  ...
  if IsLanded then
    Exit;
  if AreAllKeysFetched(AGroupInfo) then
  begin
    if KeysByIndex.TryGetKeyByValue(AKey, AIndex) then
    begin
      Result := AIndex;
...
begin
  ...
  begin
    ...
    try
      ...
      AMinPosInGroupToFetch := System.Math.Max(AStartPopulatedIndex - AGroupInfo.TopDataRowIndex + 1, 0);
      AMaxPosInGroupToFetch := System.Math.Min(AFinishPopulatedIndex - AGroupInfo.TopDataRowIndex,
        AGroupInfo.DataRowCount) - 1;
      AMaxObservableTake := Regressor.GetMaxObservableTake;
      if AMaxObservableTake < 0 then
        AMaxObservableTake := 0;
      AMaxAllowedTake := System.Math.Min(MagicNumberTakeKeysUpperLimitAfterSkip,
        System.Math.Max(AMaxObservableTake, 1024) * LongInt(4));
...
begin
  System.SysUtils.DateTimeToString(Result, GetDateTimeFormat, ADateTime);
...
const
  SummaryKindMap: array[TcxSummaryKind] of string = ('count(%s)', 'sum(%s)',
    'min(%s)', 'max(%s)', 'count(%s)', 'avg(%s)', 'order(%s)');
begin
...
begin
  ...
  FFindCriteria.Filter.OnChanged := FilterChangedHandler;
...
procedure TdxServerModeCustomDataSource.Apply(AFilter: TdxServerModeCriteria;
  AFindCriteria: TdxServerModeFindCriteria; ASortInfo: TdxServerModeSortInfoDescriptors;
  ...
  procedure UpdateFindCriteria;
  begin
    if not FindCriteria.IsEqual(AFindCriteria) then
      FindCriteria.AssignConditions(AFindCriteria);
  end;
  ...
  begin
    ...
    if not FindCriteria.Filter.IsEmpty then
      GeneralFilter.AndCriteria(FindCriteria.Filter);
  end;
...
function TdxServerModeCustomDataSource.CreateFindCriteria: TdxServerModeFindCriteria;
begin
  Result := TdxServerModeFindCriteria.Create(Self);
end;
...
var
  AIsConsistentCache: Boolean;
begin
  DataController.BeginUpdate;
  try
    Refresh;
    AIsConsistentCache := IsConsistentCache;
    if AIsConsistentCache then
      DataController.DataControllerInfo.ApplyRecoveryInfo;
  finally
    DataController.EndUpdate;
  end;
  if AIsConsistentCache and Assigned(DataController.OnSetRecoveryState) then
    DataController.OnSetRecoveryState(DataController.DataControllerInfo.FRecoveryFocusingInfo);
end;
...
begin
  ...
  FKey := Unassigned;
  FDataExpandingInfo := TdxFastObjectList.Create;
end;

destructor TdxServerModeDataFocusingInfo.Destroy;
begin
  FreeAndNil(FDataExpandingInfo);
  inherited;
end;
...
begin
  ...
  begin
    ...
    DataControllerInfo.ApplyRecoveryGroupInfo(AGroup);
...
procedure TdxServerModeDataControllerInfo.ConvertFindCriteria(ADestination: TdxServerModeFindCriteria;
  ASource: TdxServerModeDataFindCriteria);
begin
...
function TdxServerModeDataControllerInfo.CreateFindCriteria: TdxServerModeFindCriteria;
begin
  Result := DataSource.CreateFindCriteria;
  if DataController.FindCriteria.IsActive then
    ConvertFindCriteria(Result, DataController.FindCriteria);
end;
...
var
  ...
  ACriteria: TdxServerModeCriteria;
  AFindCriteria: TdxServerModeFindCriteria;
begin
  ACriteria := CreateCriteria;
  try
    AFindCriteria := CreateFindCriteria;
    try
      ...
      try
        ...
        try
          ...
          try
            ...
            try
              DataController.DataSource.Apply(ACriteria, AFindCriteria, ASortInfo, GroupingFieldList.Count, ASummaries, ATotalSummaries);
            finally
...
procedure TdxServerModeDataControllerInfo.ApplyRecoveryGroupInfo(AGroup: TdxServerModeDataGroupInfo);
var
  AExpandingInfo: TdxServerModeDataExpandingInfo;
  I: Integer;
begin
  for I := FRecoveryFocusingInfo.FDataExpandingInfo.Count - 1 downto 0 do
  begin
    AExpandingInfo := FRecoveryFocusingInfo.FDataExpandingInfo.List[I];
    if AExpandingInfo.Level = AGroup.ServerGroupInfo.Level then
    begin
      if AExpandingInfo.Level > 0 then
        if not VarEquals(AExpandingInfo.ParentGroupValue, AGroup.ServerGroupInfo.Parent.GroupValue) then
          Continue;
      if VarEquals(AExpandingInfo.Value, AGroup.ServerGroupInfo.GroupValue) then
      begin
        AGroup.Expanded := True;
        FRecoveryFocusingInfo.FDataExpandingInfo.Delete(I);
        Exit
      end;
    end;
  end;
...
var
  I: Integer;
  ADataGroupInfo: TdxServerModeDataGroupInfo;
  AItem: TdxServerModeDataExpandingInfo;
begin
  FRecoveryFocusingInfo.Assign(PrevFocusingInfo);
  FRecoveryFocusingInfo.FDataExpandingInfo.Clear;
  if Assigned(DataController.OnGetRecoveryState) then
    DataController.OnGetRecoveryState(DataController.DataControllerInfo.FRecoveryFocusingInfo);
  if DataController.GetSaveExpandingSetting then
    for I := 0 to DataGroups.Count - 1 do
    begin
      ADataGroupInfo := DataGroups[I];
      if not ADataGroupInfo.Expanded then
        Continue;
      AItem := TdxServerModeDataExpandingInfo.Create;
      AItem.GroupIndex := I;
      AItem.Value := ADataGroupInfo.FServerGroupInfo.GroupValue;
      AItem.State := [eisExpanded];
      AItem.Level := ADataGroupInfo.FServerGroupInfo.Level;
      if ADataGroupInfo.FServerGroupInfo.Parent <> nil then
        AItem.ParentGroupValue := ADataGroupInfo.FServerGroupInfo.Parent.GroupValue;
      FRecoveryFocusingInfo.FDataExpandingInfo.Add(AItem);
    end;
end;
...
function TdxServerModeDataFindCriteria.GetEscapedCondition(
...
function TdxServerModeDataFindCriteria.GetEscapeWildcard: Char;
begin
  Result := #0;
end;

function TdxServerModeDataFindCriteria.IsBehaviorSupported(
  const AValue: TcxDataFindCriteriaBehavior): Boolean;
begin
  Result := AValue <> fcbSearch;
end;

function TdxServerModeDataFindCriteria.IsConditionsLowerCase: Boolean;
begin
  Result := False;
end;

function TdxServerModeDataFindCriteria.UseMatches: Boolean;
begin
  Result := False;
end;

function TdxServerModeDataFindCriteria.GetDataController: TdxServerModeDataController;
begin
  Result := TdxServerModeDataController(inherited DataController);
end;
...
function TdxServerModeDataController.CreateFindCriteria: TcxDataFindCriteria;
begin
  Result := TdxServerModeDataFindCriteria.Create(Self);
end;
...
function TdxServerModeDataController.GetInternalErrorCode(ARecordIndex,
...
begin
  Result := not GetKeepFocusedRowOnUpdatingSetting and
   (GroupingChangedFlag or SortingChangedFlag or FilteredRecordCountChanged or LoadedStorage);
end;
...
procedure TdxServerModeDataController.PopulateFilterValues(AList: TcxDataFilterValueList; AItemIndex: Integer;
  ACriteria: TcxFilterCriteria; var AUseFilteredRecords: Boolean; out ANullExists: Boolean;
  AUniqueOnly: Boolean; AFilteredValuesShowFilteredItemsOnly: Boolean);
begin
  if (DataSource <> nil) and (Fields[AItemIndex].Field <> nil) and DataSource.Active then
    ...
    inherited PopulateFilterValues(AList, AItemIndex, ACriteria, AUseFilteredRecords, ANullExists, AUniqueOnly, AFilteredValuesShowFilteredItemsOnly);
end;
...
function TdxServerModeDataController.GetFindCriteria: TdxServerModeDataFindCriteria;
begin
  Result := TdxServerModeDataFindCriteria(inherited FindCriteria);
end;
...
function TdxServerModeDataController.GetIsProviderMode: Boolean;
begin
  Result := True;
end;
